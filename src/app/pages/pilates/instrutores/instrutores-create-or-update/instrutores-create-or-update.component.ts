...
@Component({
  ...
  templateUrl: './instrutores-create-or-update.component.html',
  styleUrls: ['./instrutores-create-or-update.component.css']
})
export class InstrutoresCreateOrUpdateComponent extends CreateOrUpdate implements OnInit {
  ...
  findById(id: string) {
    this.usuariosService.findById(id).pipe(take(1)).subscribe((responseApi: ResponseApi) => {
      ...
      this.formulario = this.formBuilder.group({
        ...
        comissao: [this.user.comissao],
        url_perfil: [this.user.url_perfil],
      })
      this.chageCitys()
      this.id = this.formulario.value.id
    }, err => {
      this.openModal(`Falha ao localizar instrutor!! (${err['error']['errors'][0]})`, MsgType.ERROR)
    })
  }
